...
- **GUI-фреймворк:** платформа решена — десктоп, нативный Rust-GUI. Открыт выбор
  фреймворка (gpui / iced / egui) — решить прототипом живого markdown-редактора
  перед началом основной разработки.
- **Жёсткость WIP-лимита:** запрет или сильная рекомендация (второй слот с трением)?
  Начать с жёсткого — ослабить проще, чем ужесточить.
- **Экспорт:** достаточно ли «копировать как markdown», или нужен экспорт в файл?
- **Режим «Пишу»:** насколько жёстко ограничивать правку назад — только неудобством
  (не видно предыдущего текста) или механически (нельзя ставить курсор выше)?
