...
# clone of this repository (brew-install design.md, D5) — and its own setup,
# which is the one line that finishes an install (guided-install design.md, D1).
install -m 755 "$root/scripts/login-item.sh" "$bundle/Contents/Resources/login-item"
install -m 755 "$root/scripts/setup.sh" "$bundle/Contents/Resources/esse-setup"
