...
Клавиши в редакторе — обычные: стрелки и `shift`+стрелки, `alt`+стрелки по
словам, `home`/`end` по видимой строке, `cmd-c`/`cmd-x`/`cmd-v`, `cmd-z` /
`cmd-shift-z`, `cmd-a`, `cmd-e` — сменить режим, `esc` — выход.
