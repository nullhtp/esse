...
//!
//! Offsets here are relative to the start of the line, in bytes.
...
/// A line as it appears on screen, plus the map back to the source line.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl DisplayLine {
    ...
    pub fn new(line: &str, plan: &RenderPlan) -> Self {
        ...
        DisplayLine { text, segments, heading: plan.heading, source_len: line.len() }
    }
    ...
    /// Style covering a display offset, for painting runs.
    pub fn style_at(&self, display: usize) -> Style {
        self.segments
            .iter()
            .find(|s| s.display.contains(&display))
            .map(|s| s.style)
            .unwrap_or_default()
    }
}
...
mod tests {
    ...
    fn clicks_map_back_into_the_source() {
        ...
        assert_eq!(display.source_offset(2), 2, "left edge of 'bold' is before `**`");
        assert_eq!(display.source_offset(6), 8, "right edge is before the closing `**`");
    }
    ...
    #[test]
    fn styles_come_back_per_display_offset() {
        let line = "a **b** *i*";
        let display = styled(line);
        assert_eq!(display.text, "a b i");

        assert!(!display.style_at(0).bold);
        assert!(display.style_at(2).bold);
        assert!(display.style_at(4).italic);
    }
}
